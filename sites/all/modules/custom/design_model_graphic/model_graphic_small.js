(function($) {
  Drupal.behaviors.designModelGraphic = { 
    attach: function(context, settings) {
      ...
      var paperDiv = $('.model-graphic-small .pane-content')[0];

      paper = Raphael(paperDiv, 300, 250);

      // Create the middle bubble.
      var model = paper.circle(paper.width * .35, paper.height * .5, 30);
      ...
      function createPhaseBubble(link, xval, yval) {
        ...
        var phase = paper.circle(xval, yval, 25);
      ...
      function circleHover(element, xval, yval) {
        element.hover(function() {
          shadow = paper.circle(xval, yval, 24);
          shadow.toBack();
          g = shadow.glow({color: "rgb(131, 131, 131)", width: 20});
          g.toBack();
        }, function() {
          g.remove();
        });
